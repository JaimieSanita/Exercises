...
public class Exercises {
	 ...
	 * 4]) → true arrayFront9([1, 2, 3, 4, 9]) → false arrayFront9([1, 2, 3, 4,
	 * 5]) → false
	 */
	 ...
	 * somewhere. array123([1, 1, 2, 3, 1]) → true array123([1, 1, 2, 4, 1]) →
	 * false array123([1, 1, 2, 1, 2, 3]) → true
	 */
	...
	public int[] makeLast(int[] nums) {
		...
		int newestArrayLength = numsLength * 2;
		int[] newestArray = new int[newestArrayLength];

		newestArray[newestArray.length - 1] = nums[nums.length - 1];
	 ...
	 * The array will be length 0, 1, or 2. double23([2, 2]) → true double23([3,
	 * 3]) → true double23([2, 3]) → false
	 */
	 ...
	 * 2, 3]) → [1, 2, 0] fix23([2, 3, 5]) → [2, 0, 5] fix23([1, 2, 1]) → [1,
	 * 2, 1]
	 */
	 ...
	 * a tie, return a. biggerTwo([1, 2], [3, 4]) → [3, 4] biggerTwo([3, 4], [1,
	 * 2]) → [3, 4] biggerTwo([1, 1], [1, 2]) → [1, 2]
	 */
	...
	public int[] makeMiddle(int[] nums) {
		...
		newMiddle[0] = nums[(nums.length / 2) - 1];
		newMiddle[1] = nums[(nums.length / 2)];
	 ...
	 * 2, 3, 4]) → [4, 2, 3, 1] swapEnds([1, 2, 3]) → [3, 2, 1] swapEnds([8, 6,
	 * 7, 9, 5]) → [5, 6, 7, 9, 8]
	 */
	public int[] swapEnds(int[] nums) {
		int length = nums.length;

		int[] newSwap = new int[nums.length];
		...
		if (nums.length >= 1) {
			...
			newSwap[0] = nums[nums.length - 1];
			newSwap[newSwap.length - 1] = nums[0];
		}
	 ...
	 * somewhere later in the array. has12([1, 3, 2]) → true has12([3, 1, 2])
	 * →true has12([3, 1, 4, 5, 2]) → true
	 */
	public boolean has12(int[] nums) {
	
	
		// one included; start at false b/c default & change true in method
		boolean oneIncluded = false;
		
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] == 1) {
				 oneIncluded = true;
			}
			if (oneIncluded && nums[i] == 2) {
				return true;
			}
		}
	 ...
	 * fizzArray3(5, 10) → [5, 6, 7, 8, 9] fizzArray3(11, 18) → [11, 12, 13, 14,
	 * 15, 16, 17] fizzArray3(1, 3) → [1, 2]
	 */
	public int[] fizzArray3(int start, int end) {
		int length = end - start;
		int[] newFizzy = new int[length];
	 ...
	 * than one of the others. lessBy10(1, 7, 11) → true lessBy10(1, 7, 10)
	 * →false lessBy10(11, 1, 7) → true
	 */
	 ...
	 * 21 without going over. Return 0 if they both go over. blackjack(19, 21) →
	 * 21 blackjack(21, 19) → 21 blackjack(19, 22) → 19
	 */
	...
	public boolean evenlySpaced(int a, int b, int c) {
		...
		int diff2 = 0;
		int diff3 = 0;
		// if all same #, 0 diff therefore true;
		if (a == b && b == c) {
			return true;
			// if two same #, 0 diff b/w 2 & diff for 3rd
		}
		if (a == b || b == c || a == c) {
			return false;
		}
		// find absolute difference between ints
		diff1 = Math.abs(a - b);
		diff2 = Math.abs(b - c);
		diff3 = Math.abs(a - c);
		// a-b == b-c true
