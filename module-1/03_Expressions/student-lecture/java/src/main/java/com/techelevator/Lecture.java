...
public class Lecture {
    ...
    */
	...
	//access(public*)	return type(type of answers being returned(int))  method name argument  	list 	start a block
    ...
    */
    public boolean returnTrueWhenOneEqualsOne() {
    	//redundant to have "return true;" because if (1 == 1) already is a boolean that will output true or false
        if (1 == 1) {
            return true;
        }

        return false; //dead code because this line will never be reached because always will return true
    }
    //
    //public boolean returnTrueWhenOneEqualsOneClean()
    //	return 1 == 1;
    /*
    ...
    */
    public boolean returnTrueWhenGreaterThanFive(int number) {
    	//simpler version
    	//return number >5; then do not need if statements
        if (number > 5) {
        	return true;
        } else {
        	return false;
