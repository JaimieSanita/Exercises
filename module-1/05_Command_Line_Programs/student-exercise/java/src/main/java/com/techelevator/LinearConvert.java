...
public class LinearConvert {
	...
	public static void main(String[] args) {
		...
		String lengthEntered= scanner.nextLine();
		double lengthEnteredAmount= Double.parseDouble(lengthEntered);
		
		System.out.print("Is the measurement in (m)eter, or (f)eet? ");
		String metersOrFeet= scanner.nextLine();

		boolean isFeet = metersOrFeet.equals("f");
		if (isFeet) {
			double feetToMeters = lengthEnteredAmount * 3.2808399;
			System.out.println(lengthEnteredAmount + "f is " + feetToMeters + "m.");
		}else {
			double metersToFeet = lengthEnteredAmount * 0.3048;
			System.out.println(lengthEnteredAmount + "m is " + " " + metersToFeet + "f.");
		} 
	}
